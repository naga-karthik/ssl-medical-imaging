...
class SegUnetEncoder_and_ProjectorG1(nn.Module):
    ...
    """
    def __init__(self, in_channels=1, base_n_filter=4, fc_units=1024, g1_out_dim=128):
        ...
        self.projector_g1 = nn.Sequential(
            nn.Linear(base_n_filter*32*(3*3), fc_units),    # back of a napkin math gives final enc6 output as 3x3 with base_n_filtersx32 filters
            nn.ReLU(inplace=True),
            nn.Linear(fc_units, g1_out_dim)
        )

        # Level 1 context pathway
        self.conv2d_c1_1 = conv_bnorm_relu(in_channels, base_n_filter)
        self.conv2d_c1_2 = conv_bnorm_relu(base_n_filter, base_n_filter)

        # Level 2 context pathway
        self.conv2d_c2_1 = conv_bnorm_relu(base_n_filter, base_n_filter*2)
        self.conv2d_c2_2 = conv_bnorm_relu(base_n_filter*2, base_n_filter*2)

        # Level 3 context pathway
        self.conv2d_c3_1 = conv_bnorm_relu(base_n_filter*2, base_n_filter*4)
        self.conv2d_c3_2 = conv_bnorm_relu(base_n_filter*4, base_n_filter*4)

        # Level 4 context pathway
        self.conv2d_c4_1 = conv_bnorm_relu(base_n_filter*4, base_n_filter*8)
        self.conv2d_c4_2 = conv_bnorm_relu(base_n_filter*8, base_n_filter*8)

        # Level 5 context pathway
        self.conv2d_c5_1 = conv_bnorm_relu(base_n_filter*8, base_n_filter*16)
        self.conv2d_c5_2 = conv_bnorm_relu(base_n_filter*16, base_n_filter*16)

        # Level 6 context pathway, level 0 localization pathway
        self.conv2d_c6_1 = conv_bnorm_relu(base_n_filter*16, base_n_filter*32)
        self.conv2d_c6_2 = conv_bnorm_relu(base_n_filter*32, base_n_filter*32)
        self.upsample_conv2d_block_l0 = upsample_conv_bnorm_relu(base_n_filter*32, base_n_filter*16)
...
if __name__ == "__main__":
    ...
    model = SegUnetEncoder_and_ProjectorG1(in_channels=1, base_n_filter=4, g1_out_dim=128)
